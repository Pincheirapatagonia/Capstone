...
class PID:
    ...
    def update(self, delta_time, x, y):
        ...
        # (abs(x-self.x_target) < self.tolpixels)
        if self.theta_err == 0 or ((x > (self.x_target-self.tolpixels)) and (x < (self.x_target+self.tolpixels))):
